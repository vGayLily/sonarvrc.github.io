---
title: Staff list
layout: default
nav_order: 0
parent: Role Descriptions
---
# SONAR Staff list
Welcome to the SONAR Staff list. This page lists all the teams within SONAR and the people filling roles in them. Names/tags are those used in the Discord server.

If you want to apply for any of these teams please create a ticket in **[#tickets](https://discord.com/channels/1144683882336231524/1163422864637169754)** in the Discord server.

## Open positions
### Media team - Social media manager
We could use some help finding our voice on Bluesky and TikTok!

## Commanders
@**Arkui** - Development \
**Llama** - Worlds \
@**magictwin** - Media, Organisation \
@**Mitsync** - Moderation \
**Monty!** / @dahottah - Events \
**v·lily** / @vlily - Events

## Organisation
@**magictwin** \
**Llama** \
@**Mitsync**

## World developers
**Llama** - Underpass and many others \
@**magictwin** - Substation \
**Monty!** / @dahottah - Threshold

## Events
**Monty!** / @dahottah \
**v·lily** / @vlily  \
**Dax** / @aussiedax \
**Daxan** / @daxanoxe \
@**LunaVixi** \
**Pixel ❰ ║▌- ▪/▪ ❱** / @pixeldoesstuff \
**Random Memories** / @randommemories.

## Media
@**magictwin** \
**Mia** \
**pixel** / @pixelmeows \
**Pixel ❰ ║▌- ▪/▪ ❱** / @pixeldoesstuff \
@**Regar** \
@**Silverplushie** \
**v·lily** / @vlily

## Moderators
@**Mitsync** \
**Barror1** / @cath20021 \
**howl** \
**Llama** \
@**LunaVixi** \
@**magictwin** \
**Mia** \
**Pixel ❰ ║▌- ▪/▪ ❱** / @pixeldoesstuff \
@**Riksaah** \
@**Schun_justschun** \
**Shrooky** / @shrookywaff \
**SilverFoxx**

## Development
@**Arkui** \
@**magictwin** \
**v·lily** / @vlily

## On shoreleave
**Glitchy** / @kindaglitchy - Moderator \
@**subdefense** - Events

## In memory of
V N I V Ꞁ Ǝ

Last update: 09-08-2026
